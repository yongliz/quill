/**
 * Copyright(c) 2020-present, Odysseas Georgoudis & quill2 contributors.
...
/**
 * When QUILL_DISABLE_NON_PREFIXED_MACROS is enabled, any macros starting with `LOG_*` are removed.
 * Only the equivalent `QUILL_LOG_*` macros will remain available.
 * This is only useful in the case the original macro names collide with an existing logging library
 */
 ...
 */
constexpr bool USE_CHRONO_CLOCK{false};
 ...
 */
constexpr bool USE_BOUNDED_QUEUE{false};
 ...
 */
constexpr size_t QUEUE_CAPACITY{262'144};
 ...
 * @note: This should be switchable even after quill is already installed as a static or shared library.
 */
constexpr std::chrono::milliseconds RDTSC_RESYNC_INTERVAL{700};
 ...
 *
 * UNSAFE_COPY is not recommended as the user should be never careful when logging user
 * defined types.
 * When UNSAFE_COPY is enabled quill will no longer perform any checks and will copy any user
 * defined types that are copy constructible to the queue and will format those copies later at the
 * backend logging thread.
 * If you enable UNSAFE_COPY you no longer have to tag your classes or specialize copy_loggable.
 */
constexpr bool UNSAFE_COPY{false};
} // namespace defaults
 ...
 * Pull any tweaks
 */
#if __has_include(<QuillTweaks.h>)
  #include <QuillTweaks.h>
#elif __has_include("QuillTweaks.h")
  #include "QuillTweaks.h"
#endif