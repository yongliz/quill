...
private:
  ...
  alignas(config::CACHELINE_SIZE) std::atomic<bool> _new_thread_context{false};
   ...
   */
  alignas(config::CACHELINE_SIZE) std::atomic<uint8_t> _invalid_thread_context{0};
  char _pad0[config::CACHELINE_SIZE - sizeof(std::atomic<uint8_t>)] = "\0";
};
} // namespace detail
} // namespace quill